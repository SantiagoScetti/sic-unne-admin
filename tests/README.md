...
npx vitest run tests/domain/denuncia/Denuncia.test.ts   # un solo archivo
```
...
|---|---:|---|
| `tests/domain/denuncia/Denuncia.test.ts` | 7 | **Patrón Estado**: un denuncia `Pendiente` puede resolverse/desestimarse; uno `Resuelto`/`Desestimado` ya no (lanza `DenunciaYaProcesadaError` → 409); `emisorEsSistema()`; `aFilaPersistible()`. |
| `tests/domain/denuncia/ServicioResolucionDenuncia.test.ts` | 4 | **Orquestador C-01**: al resolver con suspensión, delega en cada repositorio (guardar, suspender, notificar, auditar); usa admin por defecto si no se pasa; ante un denuncia ya procesado **no escribe nada**; `desestimar()` audita sin sancionar. |
