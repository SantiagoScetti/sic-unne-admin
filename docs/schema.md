...
- auth_id (uuid) [FK -> auth.users.id] - El vínculo con el sistema de autenticación.
- nombre (character varying)
- apellido (character varying)
- documento (integer)
- correo (character varying)
- estado (character varying) - (Activo, Suspendido, etc.)
- rol (character varying) - (Administrador, Estudiante)
- id_carrera (integer) [FK -> carrera.id_carrera]
- fecha_suspension_hasta (timestamp)
  Foreign Keys
- id_carrera → carrera.id_carrera
- auth_id → auth.users.id (Gestionado por Supabase Auth)
...
usuario: id_carrera → carrera.id_carrera
