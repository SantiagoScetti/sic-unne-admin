...
export type AccionDenuncia = {
  accion: 'Suspender Temporalmente' | 'Suspender Indefinidamente' | 'Enviar aviso';
  fechaHasta?: string;
  observaciones: string;
};

type AccionDenunciaModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSave: (accion: AccionDenuncia) => void;
  onEliminar?: () => void;
...
const AccionDenunciaModal = ({
  ...
  denuncia,
  isSaving = false,
}: AccionDenunciaModalProps) => {

  const [accion, setAccion] = useState<AccionDenuncia['accion']>('Enviar aviso');
  ...
  // ── Datos derivados ──
  const esEmisoresSistema = !denuncia.emisor_id || !denuncia.emisor;
  const denunciantePor = esEmisoresSistema
    ? 'Sistema'
    : `${denuncia.emisor!.apellido}, ${denuncia.emisor!.nombre}${denuncia.emisor!.documento ? ` — DNI ${denuncia.emisor!.documento}` : ''}`;

  const receptor = denuncia.receptor;
  const denunciadoA = receptor
    ? `${receptor.apellido}, ${receptor.nombre}${receptor.documento ? ` — DNI ${receptor.documento}` : ''}`
    : 'Usuario desconocido';

  const fechaDenuncia = new Date(denuncia.fecha_alta).toLocaleDateString('es-AR', {
  ...
  const badge = badgeColor[denuncia.estado] ?? badgeColor['Desestimado'];
      ...
      <div style={{ ...modalStyle, width: '520px', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', borderBottom: '2px solid #e2e8f0', paddingBottom: '12px' }}>
          <h2 style={{ margin: 0, color: '#2d3748', fontSize: '1.2rem' }}>
            Gestionar Denuncia <span style={{ color: '#718096', fontWeight: '400' }}>#{denuncia.id_denuncia}</span>
          </h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={infoBlockStyle}>
            <p style={infoLabelStyle}>Denunciante</p>
          ...
          {/* ── A quién se denunció ── */}
          <div style={infoBlockStyle}>
            <p style={infoLabelStyle}>Estudiante denunciado</p>
            <p style={{ ...infoValueStyle, color: '#c53030' }}>{denunciadoA}</p>
            <p style={{ margin: '2px 0 0', fontSize: '0.75rem', color: '#a0aec0' }}>Fecha de la denuncia: {fechaDenuncia}</p>
          </div>

          {/* ── Descripción / motivo ── */}
          <div>
            <label style={labelStyle}>Descripción de la denuncia</label>
            <div style={{ ...inputBaseStyle, minHeight: '70px', backgroundColor: '#f8fafc', color: '#4a5568', lineHeight: '1.5', whiteSpace: 'pre-wrap' }}>
              {denuncia.motivo || <span style={{ fontStyle: 'italic', color: '#a0aec0' }}>Sin descripción</span>}
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ebf8ff', border: '1px solid #bee3f8', borderRadius: '8px', padding: '12px 14px', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
            ...
            <p style={{ margin: 0, fontSize: '0.82rem', color: '#2b6cb0', lineHeight: '1.5' }}>
              ...
              {esEmisoresSistema
                ? 'el estudiante denunciado será notificado automáticamente.'
                : 'tanto el estudiante denunciado como quien realizó la denuncia serán notificados automáticamente.'}
            </p>
          </div>
...
export default AccionDenunciaModal;
