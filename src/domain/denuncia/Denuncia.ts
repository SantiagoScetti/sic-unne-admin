import type { EstadoDenuncia } from './estados/EstadoDenuncia';
import { crearEstado } from './estados';
import type { AccionTomada, EstadoDenunciaNombre, DenunciaData } from './tipos';

// =============================================================================
// Entidad de dominio: Denuncia.
//
...
export class Denuncia {
  ...
  private _estado: EstadoDenuncia;

  constructor(data: DenunciaData) {
  ...
  // ── Consultas ──────────────────────────────────────────────────────────
  get estado(): EstadoDenunciaNombre { return this._estado.nombre; }
  ...
  transicionarA(nuevoEstado: EstadoDenuncia): void { this._estado = nuevoEstado; }
  registrarAccion(accion: AccionTomada): void { this._accionTomada = accion; }

  // ── Operaciones de negocio (delegan en el Estado actual) ────────────────
  /** Resuelve la denuncia aplicando una acción. Lanza si no está 'Pendiente'. */
  resolver(accion: AccionTomada): void { this._estado.resolver(this, accion); }
  /** Desestima la denuncia. Lanza si no está 'Pendiente'. */
  desestimar(): void { this._estado.desestimar(this); }
  /** Asigna el administrador que gestionó la denuncia. */
